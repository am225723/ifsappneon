const MEANINGFUL_SHORT_RESPONSES = new Set(['yes', 'no', 'ok', 'fear', 'anger', 'shame', 'sad', 'mad', 'calm', 'joy', 'self', 'curious', 'compassion', 'clarity', 'courage', 'confidence', 'connected', 'creative', 'presence']);
...
function isDateTimeOnly(value) {
  ...
  return /^\d{1,2}:\d{2}(?::\d{2})?\s?(am|pm)?$/i.test(text) || /^\d{4}-\d{2}-\d{2}(?:[t\s]\d{2}:\d{2}(?::\d{2}(?:\.\d{1,6})?)?(?:z|[+-]\d{2}:?\d{2})?)?$/i.test(text);
}
