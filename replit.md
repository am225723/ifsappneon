...
- **Homework Assignment System**: Advisors assign categorized assigned practice with priorities and due dates; clients complete assignments with reflection notes. AI-assisted Assigned IFS Practice generation via server-side OpenRouter API (`src/lib/assigned practiceAI.js` -> `/api/ai-assigned-practice`): "Generate One" creates a single wound-personalized assignment that auto-fills the form for review; "Generate Set of 4" produces multiple suggestions displayed as selectable cards. Supports optional category focus and freeform advisor guidance. Automatically detects client wound type from assessment data.
...
- **Push Notification Reminders**: OneSignal-integrated notification system with user tagging on login. Configurable reminder categories (daily check-in, assigned practice, journal streak, meditation, weekly reflection, advisor messages). Preferences saved to `ifs_clients.notification_preferences` JSONB column with localStorage fallback. Settings UI in `src/components/NotificationSettings.jsx`, integrated into Settings page. Library: `src/lib/pushNotifications.js`. DB migration: `021_add_notification_preferences.sql`.
