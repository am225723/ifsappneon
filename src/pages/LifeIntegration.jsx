...
function ReflectionCard({ reflection }) {
  ...
  return (
    <article className="rounded-3xl border border-brand-stone-100 bg-white/80 p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md dark:border-slate-800 dark:bg-slate-900/50">
      <div className="mb-3 flex items-start justify-between gap-3">
        ...
        <span className={`shrink-0 rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide ${reflection.shared_with_advisor ? 'bg-brand-emerald-50 text-brand-emerald-700 dark:bg-brand-emerald-950/30 dark:text-brand-emerald-100' : 'bg-brand-stone-100 text-brand-stone-600 dark:bg-slate-800 dark:text-slate-300'}`}>
          {reflection.shared_with_advisor ? 'Shared' : 'Private'}
        </span>
      </div>
        ...
        <section className="mt-12 grid gap-8 lg:grid-cols-[1fr_0.65fr]">
          ...
          <aside className="space-y-4">
            <div className="soft-card bg-white/85 p-6 dark:bg-brand-cardDark/90">
              ...
              <p className="mt-2 text-sm leading-relaxed text-brand-stone-600 dark:text-slate-400">Your reflections are private unless you choose to share them with your Advisor. Sharing is optional and explicit.</p>
            </div>
            <div className="soft-card bg-white/85 p-6 dark:bg-brand-cardDark/90">
              ...
              <p className="mt-2 text-sm leading-relaxed text-brand-stone-600 dark:text-slate-400">These practices support reflection and IFS self-guidance. They are not monitored for emergencies.</p>
