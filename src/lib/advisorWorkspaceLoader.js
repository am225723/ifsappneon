...
// The Quick Practice Generator (clinical-practice tab) previously built
// drafts from hardcoded local string templates and only ever pushed an
// assigned practice into local React state — nothing was ever generated by
// an LLM or persisted. api/ai-assigned-practice.js (via homeworkAI.js) and
// the ifs_therapy_homework table already do both of these for real, just
// only reachable from the standalone TherapistHomework.jsx page.
export async function generateWorkspacePractice(params) {
  try {
    const result = await generateHomework(params);
    return { data: result, error: null };
  } catch (error) {
    return { data: null, error: { message: error.message || 'Unable to generate a practice draft.' } };
  }
}

export async function generateWorkspacePracticeBatch(params) {
  try {
    const results = await generateHomeworkBatch(params);
    return { data: results, error: null };
  } catch (error) {
    return { data: null, error: { message: error.message || 'Unable to generate practice drafts.' } };
  }
}

// Mirrors TherapistHomework.jsx's own insert exactly, including its
// graceful fallback when the activity_blocks column isn't migrated yet in
// a given environment — the practice is still saved, just without the
// structured interactive blocks.
export async function assignWorkspacePractice({ clientId, therapistId, title, description, category, priority, activityBlocks }) {
  if (!clientId || !therapistId || !title) return { error: { message: 'Missing client, therapist, or title' } };
  ...
  const hasActivityBlocks = Boolean(activityBlocks?.length);
  const payload = hasActivityBlocks ? { ...basePayload, activity_blocks: activityBlocks } : basePayload;

  const { data, error } = await supabase.from('ifs_therapy_homework').insert(payload).select().single();
  if (error && hasActivityBlocks && isMissingWorksheetPersistenceColumn(error, ['activity_blocks'])) {
    const fallback = await supabase.from('ifs_therapy_homework').insert(basePayload).select().single();
    if (fallback.error) return { error: fallback.error };
    return { data: fallback.data, warning: `${WORKSHEET_MIGRATION_ADMIN_WARNING} The assignment was saved without structured activity blocks.` };
  }
