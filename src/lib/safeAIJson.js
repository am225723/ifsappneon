...
function parseStringValue(value) {
  ...
  if (trimmed.startsWith('{') || trimmed.startsWith('[') || /^```/.test(trimmed)) {
    return safeParseAIJson(trimmed, value);
  }
  return value;
}
...
function normalizeBlock(raw, index) {
  ...
  return {
    ...block,
    type,
    id: String(block.id || block.widgetId || `${type}_${index + 1}`)
  };
}
...
export function normalizeActivityBlocks(value) {
  ...
  const source = Array.isArray(parsed)
    ? parsed
    : Array.isArray(parsed?.activity_blocks)
      ? parsed.activity_blocks
      : Array.isArray(parsed?.activityBlocks)
        ? parsed.activityBlocks
...
export function splitDescriptionAndActivityBlocks(text) {
  ...
  const description = source.slice(0, match.index).trim();
  const jsonText = source.slice(match.index + match[0].length);
  return { description, activityBlocks: normalizeActivityBlocks(jsonText) };
}
